...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await db_api.add_user(user_id=user_id)
...
async def start_learning(message: Message, state: FSMContext):
    user_id = message.from_user.id
    user_words = await db_api.get_word(user_id=user_id)
    if not user_words:
...
async def show_word(message: Message, state: FSMContext):
    ...
    if not remaining_words:
        ...
        await state.clear()
        return
    word = remaining_words[random.randint(0, len(remaining_words)-1)]
    ...
    await message.answer(f"Как переводится слово: {word}?", reply_markup=keyboard)
    learned.add(word)
    await state.update_data(current_correct=correct_translation, current_word=word, learned=learned)
...
async def check_answer(message: Message, state: FSMContext):
    ...
    if user_answer == '❌Удалить слово (это прервет обучение)':
    ...
    if user_answer == '➕Добавить слово (это прервет обучение)':
        await state.clear()
...
async def start_count_word(message: Message):
    len_ = len(await db_api.get_word(user_id=message.from_user.id))
...
async def del_word_step(message: Message, state: FSMContext):
    ...
    user_id = message.from_user.id
    await db_api.del_user_word(user_id=user_id, user_word=word)
    count_word = await db_api.get_word(user_id=user_id)
